...
- Build a multi-path forward-selection forest of models using `build_paths()`
- Estimate predictor stability across bootstrap samples using `stability()`
- Construct a plausible Rashomon set using `plausible_models()`
- Evaluate the best plausible model on a held-out test set using RMSE
