...
data.frame(variable = colnames(X), AIC = aics)[order(aics), ][1:8, ]
```

##  Reproducing results using `mpssAIC`
```{r}
...
plaus <- plausible_models(forest, pi = stab, Delta = 2, tau = 0.6)
plaus
```

```{r}
data.frame(variable = forest$var_names, AIC = forest$step_AICs)[order(forest$step_AICs), ][1:8, ]
```
