...
subtitle: |
  ...
  <br>
  Group 11
  [![GitHub](https://img.shields.io/badge/GitHub-Repo-blue?logo=github)](https://github.com/R-4-Data-Science/Final_project_Group11t)
...
**Note:** The implementations shown in this HTML report use internal function names 
(`multi_path_forward()`, `compute_stability()`, `select_plausible_models()`) for 
reproducibility. The installed package exports these with the assignment-specified 
names (`build_paths()`, `stability()`, `plausible_models()`) as wrapper functions.
...
## Algorithm 4 – (Optional) confusion_metrics() for logistic models
...
```
## Algorithm 5 – Full Multi-Path AIC Pipeline
...
## Binimial Regression (Synthetic)

```{r example-Bimonial}
## Complete Example – Binomial Regression (Synthetic Binary Data)

set.seed(123)

# --------------------------------------------------------------------------
# 1. Simulate synthetic binary outcome data
# --------------------------------------------------------------------------

n <- 300          # number of observations
p <- 6            # number of predictors

X_binom <- matrix(rnorm(n * p), nrow = n, ncol = p)
colnames(X_binom) <- paste0("x", 1:p)

# True coefficients (sparse)
beta_true <- c(1.0, -1.5, 0.75, 0, 0, 0)

# Linear predictor and probabilities (logistic link)
eta  <- X_binom %*% beta_true
prob <- plogis(eta)
...
# --------------------------------------------------------------------------
# 2. Run multi-path AIC pipeline (binomial)
# --------------------------------------------------------------------------

cat("\n\n")
cat(strrep("#", 70), "\n")
cat("COMPLETE EXAMPLE: BINOMIAL REGRESSION (Synthetic Binary Data)\n")
cat(strrep("#", 70), "\n\n")

result_binom <- multi_path_AIC_pipeline(
  X             = X_binom,
  y             = y_binom,
  model_type    = "binomial",
  K             = NULL,        # e.g. min(p, 10)
  ...
  delta         = 2,
  L             = 50,
  B             = 20,          # increase to 50+ for serious use
  resample_type = "bootstrap",
  m             = NULL,
  Delta         = 2,
  tau           = 0.6
...
cat("\n### FINAL RESULTS: BINOMIAL REGRESSION ###\n\n")
cat("Number of plausible models:", nrow(result_binom$plausible), "\n\n")

final_table_binom <- result_binom$plausible[
  , c("AIC", "AIC_diff", "size", "vars", "avg_stability")
]
rownames(final_table_binom) <- NULL
print(final_table_binom)

cat("\n### Stability Scores ###\n")
print(round(result_binom$stability_pi, 3))

# --------------------------------------------------------------------------
# 3. Confusion matrix and classification metrics for best plausible model
# --------------------------------------------------------------------------

cat("\n\n")
cat(strrep("=", 70), "\n")
cat("CLASSIFICATION PERFORMANCE EVALUATION (BINOMIAL)\n")
cat(strrep("=", 70), "\n\n")

if (nrow(result_binom$plausible) > 0) {
  # Best plausible model (first row)
  best_key  <- result_binom$plausible$key[1]
  best_vars <- result_binom$plausible$vars[1]
  best_idx  <- as.integer(strsplit(best_key, ",")[[1]])
  
  cat("Best Plausible Model:", best_vars, "\n\n")
  
  # Fit binomial GLM on the chosen variables
  X_best <- X_binom[, best_idx, drop = FALSE]
  df_binom <- data.frame(y = y_binom, X_best)
  fit_best_binom <- glm(y ~ ., data = df_binom, family = binomial())
  
  # Confusion matrix + metrics at cutoff 0.5
  cm_results <- confusion_metrics(
    fit    = fit_best_binom,
    y      = y_binom,
    cutoff = 0.5
  ...
  # Optional simple confusion matrix visualization
  cat("\n")
  library(graphics)
  ...
  # Background rectangles (TN / FP / FN / TP)
  rect(c(0, 1), c(0, 0), c(1, 2), c(1, 1), 
       col = c("lightgreen", "lightcoral"))
  rect(c(0, 1), c(1, 1), c(1, 2), c(2, 2), 
       col = c("lightcoral", "lightgreen"))
...
To view the vignette after installing the package:
```{r eval=FALSE}
library(mpssAIC)
...
**Location:** `mpssAIC/vignettes/diabetes_mpssAIC.Rmd`
